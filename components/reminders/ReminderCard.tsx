...
import { CheckCircle, Circle, Clock, Trash2, MessageSquare, Bell, Mail, FileText, ChevronDown } from "lucide-react";
...
export const ReminderCard = memo(function ReminderCard({ reminder, onEdit }: ReminderCardProps) {
    ...
    const hasNotes = reminder.notes && reminder.notes.trim().length > 0;
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            {hasNotes && (
                                <button
                                    onClick={toggleNotes}
                                    className={cn(
                                        "flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full transition-colors",
                                        notesExpanded
                                            ? "bg-violet-500/20 text-violet-600 dark:text-violet-400"
                                            : "bg-muted text-muted-foreground hover:bg-violet-500/10 hover:text-violet-600 dark:hover:text-violet-400"
                                    )}
                                >
